...
- [x] 头部点击/抚摸交互；
- [x] 鼠标拖动视线跟随，可单独关闭；
- [x] JP/CN/KR 语音选择；字幕支持简体中文、日文、韩文和英文；
- [x] BGM、语音独立开关与音量；
- [x] 字幕显示开关；
- [x] 模型缩放及 X/Y 偏移；
...
- 字幕缩放属性不属于 `1.0.0` 的 M3/M4 门槛；
- 字幕预设及自定义位置应覆盖超宽屏、双屏和高 DPI 下的布局回归测试。
...
| `dialoguelanguagepreset` | combo | `zh-cn` | 中/日/韩/英固定对话预设或 `Custom`；英文使用日语语音配官方英文字幕 |
| `voicelanguage` | combo | `zh-cn` | 自定义对话的中文/日文/韩文语音 |
| `showsubtitles` | bool | `true` | 自定义对话的字幕开关 |
| `subtitlelanguage` | combo | `zh-cn` | 自定义对话的主字幕语言（简体中文/日文/韩文/英文） |
| `showsecondarysubtitles` | bool | `false` | 显示字幕时可选的副字幕开关 |
| `secondarysubtitlelanguage` | combo | `ja` | 同时启用主、副字幕时显示的副字幕语言；与主语言相同时只显示主字幕 |
| `subtitlealignment` | combo | `center` | 自定义对话的居中、左侧或右侧字幕对齐 |
| `subtitleposition` | combo | `bottom-center` | 底部中央、顶部中央、屏幕中心、底部左侧或自定义位置 |
| `subtitlex` / `subtitley` | slider | `0` | 自定义字幕位置相对屏幕中心的像素偏移，范围 `-1000..1000` |
...
| `panelpositionpreset` | combo | `default` | 调试面板专用的 `Default`/`Custom` 位置与缩放预设；与模型预设相互独立 |
| `panelscale` | slider | `1.00` | 自定义面板位置与缩放时的面板尺寸，范围 `0.60–1.40` |
| `panelx` / `panely` | slider | `0` | 自定义面板位置与缩放时的 X/Y 像素偏移，范围 `-1000–1000` |
| `panellanguage` | combo | `zh-cn` | 面板启用后显示的简体中文/英文界面选项，不受 Debug 固定预设重置 |

桌面调试面板按 WE 属性页顺序复用画质、模型位置与缩放、动画与互动、音量和对话播放的设置及其显隐规则，不提供主题颜色和 `Debug` 预设。面板最前方的“面板设置”分组集中提供面板位置与缩放预设、面板尺寸、X/Y 偏移及面板语言；固定 `Default` 隐藏三个几何子项，切换 `Custom` 时恢复之前的自定义值。每个顶层设置分组和全部嵌套子分组的标题栏都是独立的手动折叠按钮，支持键盘操作和展开状态提示；折叠状态只属于当前页面 UI，不随属性、预设或“恢复 WE 设置”变化，子分组因条件暂时隐藏后重新显示时也保留原状态。音量分组统一包含静音、BGM 音量和对话音量；对话播放分组包含自动播放、语言预设，并将自定义对话的主、副字幕及布局设置集中在醒目的字幕子分组中。面板修改只作为当前实例的临时覆盖，不写回 WE 管理页；固定预设与 `Custom` 的子值恢复由同一设置适配器处理，管理页更新某个键时清除该键的临时覆盖，“恢复 WE 设置”清除全部临时覆盖。浏览器开发可用 `?debug=1` 显示面板；真实宿主启用 `debugpanelenabled` 时面板立即展开，之后可通过右上角按钮切换可见性。

后续可选属性为视线跟随强度、用户自选 BGM 文件和字幕缩放。实现 `file` 属性时需按 Wallpaper Engine 的本地路径规则转换为 `file:///` URL，并处理空值和不可播放格式；这些增强不属于 `1.0.0` 离线发行门槛。
...
- 后续补充的全球服 `CharacterDialogExcel` 为该记忆大厅提供韩文和英文游戏文本，可按事件 ID 直接采用，无需机器翻译；
...
- [x] WE 属性：入场、构图、交互、三语语音、四语字幕、BGM、FPS 上限、命中区、渲染/纹理分辨率和面板语言；
...
- [x] 固定并自动验证模型、纹理、三语语音、四语字幕、BGM 和 Spine Runtime；
...
`release/hare-camping-we-offline-v1.0.0.zip` 是当前正式离线版。之后可独立评估用户自选 BGM、对话时 BGM ducking、字幕缩放、字幕主题与淡入淡出、随机且避免连续重复的台词模式，以及更完整的动画/骨骼检查器。这些项目不影响 `1.0.0` 的离线发行结论。
