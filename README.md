...
Wallpaper Engine 中的属性标签与选项保持英文。属性按 `Visual Quality`、`Position & Scale`、`Animation & Interactions`、对话、`BGM` 和 `Debug` 的顺序排列；固定预设会隐藏所属的高级子项，选择 `Custom` 后才显示并恢复此前的自定义值。`Debug` 默认为 `Off`；启用面板后，桌面右上角会显示透明底的面板切换按钮。面板内的参数调整只覆盖当前运行实例，不会改写管理页的持久设置，“恢复 WE 设置”可清除全部临时覆盖。
