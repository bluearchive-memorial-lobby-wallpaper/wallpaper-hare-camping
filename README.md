...
每次壁纸加载都会单独记录一份日志，内容包括错误、动画切换、交互点击和配置修改。日志脚本会在应用代码之前启动，并直接保存到壁纸宿主提供的持久存储中，无需首次选择目录。调试面板顶部的“打开日志”可在面板内查看和复制最近会话，并识别未正常结束的会话；使用本地 Vite 开发或预览服务时，日志还会同步写入构建目录的 `log` 文件夹。日志功能不增加 WE 属性页选项。
